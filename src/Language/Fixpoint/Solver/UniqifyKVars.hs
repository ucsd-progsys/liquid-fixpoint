...
remakeSubst fi k su = foldl' (updateSubst k) su kDom
  where
    w    = (ws fi) M.! k
    kDom = (fst $ wrft w) : (fst <$> envCs (bs fi) (wenv w))

updateSubst :: KVar -> Subst -> Var -> Subst
updateSubst k (Su su) sym
  | sym `M.member` su = Su $ M.delete sym $ M.insert sym' (su M.! sym) su
  | otherwise         = Su $ M.insert sym' (EVar sym) su
  where
    kx   = kv k
    sym' = mapVarSymbol (`kArgSymbol` kx) sym
...
updateWfc fi w    = fi' { ws = M.insert k w' (ws fi) }
  where
    ...
    w'            = w { wenv = env', wrft = (mapVarSymbol (`kArgSymbol` kv k) v, k) }
...
accumBindsIfValid k (fi, ids) i = if renamable then accumBinds k (fi, ids) i else (fi, i : ids)
  where
    ...
    renamable = isValidInRefinements $ reftSort sr
...
accumBinds k (fi, ids) i = (fi {bs = be'}, i' : ids)
  where
    ...
    newSym    = mapVarSymbol (\x -> kArgSymbol x (kv k)) oldSym
...
isValidInRefinements (FAbs  _ _) = False
