...
init cfg si ks_ = Sol.fromList symEnv mempty keqs [] mempty ebs xEnv
  where
    keqs       = map (refine (solverFlags $ solver cfg) si qcs genv) ws `using` parList rdeepseq
...
refine :: ElabFlags -> F.SInfo a -> QCluster -> F.SEnv F.Sort -> F.WfC a -> (F.KVar, Sol.QBind)
refine ef info qs genv w = refineK ef (allowHOquals info) env qs (F.wrft w)
  where
    env             = wenvSort <> genv
...
refineK :: ElabFlags -> Bool -> F.SEnv F.Sort -> QCluster -> (F.Symbol, F.Sort, F.KVar) -> (F.KVar, Sol.QBind)
refineK ef ho env qs (v, t, k) = F.notracepp _msg (k, eqs')
   where
...
okInst ef env v t eq = isNothing tc
  where
    ...
    p             = Sol.eqPred eq
    tc            = So.checkSorted ef (F.srcSpan eq) env sr
...
lhsPred
  ...
  -> F.SimpC a
  -> F.Expr
lhsPred bindingsInSmt ef be s c = F.notracepp _msg $ fst $ apply ef g s bs
  where
...
apply :: ElabFlags -> CombinedEnv ann -> Sol.Sol a Sol.QBind -> F.IBindEnv -> ExprInfo
apply ef g s bs      = (F.conj (pks:ps), kI)   -- see [NOTE: pAnd-SLOW]
  where
    ...
    (pks, kI)     = applyKVars ef g {ceBindingsInSmt = F.emptyIBindEnv} s ks
    (ps,  ks, _)  = envConcKVars ef g s bs


envConcKVars :: ElabFlags -> CombinedEnv ann -> Sol.Sol a Sol.QBind -> F.IBindEnv -> ([F.Expr], [F.KVSub], [F.KVSub])
envConcKVars ef g s bs = (concat pss, concat kss, L.nubBy (\x y -> F.ksuKVar x == F.ksuKVar y) $ concat gss)
  where
    (pss, kss, gss) = unzip3 [ F.notracepp ("sortedReftConcKVars" ++ F.showpp sr) $ F.sortedReftConcKVars x sr | (x, sr) <- xrs ]
    xrs             = lookupBindEnvExt ef g s <$> is
    is              = F.elemsIBindEnv bs

lookupBindEnvExt :: ElabFlags -> CombinedEnv ann -> Sol.Sol a Sol.QBind -> F.BindId -> (F.Symbol, F.SortedReft)
lookupBindEnvExt ef g s i
  | Just p <- ebSol ef g {ceBindingsInSmt = F.emptyIBindEnv} s i = (x, sr { F.sr_reft = F.Reft (x, p) })
...
ebSol :: ElabFlags -> CombinedEnv ann -> Sol.Sol a Sol.QBind -> F.BindId -> Maybe F.Expr
ebSol ef g sol bindId = case  M.lookup bindId sebds of
  ...
  where
    ...
    ebReft s (i,c) = exElim (Sol.sxEnv s) (senv c) i (ebindReft ef g s c)
...
ebindReft :: ElabFlags -> CombinedEnv ann -> Sol.Sol a Sol.QBind -> F.SimpC () -> F.Pred
ebindReft ef g s c = F.pAnd [ fst $ apply ef g' s bs, F.crhs c ]
  where
...
applyKVars :: ElabFlags -> CombinedEnv ann -> Sol.Sol a Sol.QBind -> [F.KVSub] -> ExprInfo
applyKVars ef g s = mrExprInfos (applyKVar ef g s) F.pAndNoDedup mconcat

applyKVar :: ElabFlags -> CombinedEnv ann -> Sol.Sol a Sol.QBind -> F.KVSub -> ExprInfo
applyKVar ef g s ksu = case Sol.lookup s (F.ksuKVar ksu) of
  Left cs   -> hypPred ef g s ksu cs
  Right eqs -> (F.pAndNoDedup $ fst <$> Sol.qbPreds ef msg s (F.ksuSubst ksu) eqs, mempty) -- TODO: don't initialize kvars that have a hyp solution
  where
    msg     = "applyKVar: " ++ show (ceCid g)

nonCutsResult :: ElabFlags -> F.BindEnv ann -> Sol.Sol a Sol.QBind -> M.HashMap F.KVar F.Expr
nonCutsResult ef be s =
  let g = CEnv Nothing be F.emptyIBindEnv F.dummySpan F.emptyIBindEnv
   in M.mapWithKey (mkNonCutsExpr g) $ Sol.sHyp s
  where
    mkNonCutsExpr g k cs = F.pOr $ map (bareCubePred ef g s k) cs
...
bareCubePred :: ElabFlags -> CombinedEnv ann -> Sol.Sol a Sol.QBind -> F.KVar -> Sol.Cube -> F.Expr
bareCubePred ef g s k c =
  let bs = Sol.cuBinds c
      ...
      (xts, psu) = substElim ef (Sol.sEnv s) sEnv g' k su
      (p, _kI) = apply ef g' s bs'
   in F.pExist (xts ++ yts) (psu &.& p)

hypPred :: ElabFlags -> CombinedEnv ann -> Sol.Sol a Sol.QBind -> F.KVSub -> Sol.Hyp  -> ExprInfo
hypPred ef g s ksu hyp = F.pOr *** mconcatPlus $ unzip $ cubePred ef g s ksu <$> hyp
...
elabExist :: ElabFlags -> F.SrcSpan -> Sol.Sol a Sol.QBind -> [(F.Symbol, F.Sort)] -> F.Expr -> F.Expr
elabExist ef sp s xts p = F.pExist xts' p
  where
    xts'        = [ (x, elab t) | (x, t) <- xts]
    elab        = So.elaborate (So.ElabParam ef (F.atLoc sp "elabExist") env)
    env         = Sol.sEnv s

cubePred :: ElabFlags -> CombinedEnv ann -> Sol.Sol a Sol.QBind -> F.KVSub -> Sol.Cube -> ExprInfo
cubePred ef g s ksu c    = (F.notracepp "cubePred" $ elabExist ef sp s xts (psu &.& p), kI)
  where
    sp                = F.srcSpan g
    ((xts,psu,p), kI) = cubePredExc ef g s ksu c bs'
    bs'               = delCEnv s k bs
    bs                = Sol.cuBinds c
    k                 = F.ksuKVar ksu
...
cubePredExc :: ElabFlags -> CombinedEnv ann -> Sol.Sol a Sol.QBind -> F.KVSub -> Sol.Cube -> F.IBindEnv
            -> ((Binders, F.Pred, F.Pred), KInfo)

cubePredExc ef g s ksu c bs' = (cubeP, extendKInfo kI (Sol.cuTag c))
  where
    cubeP           = (xts, psu, elabExist ef sp s yts' (F.pAndNoDedup [p', psu']) )
    sp              = F.srcSpan g
    yts'            = symSorts g bs'
    g'              = addCEnv  g bs
    (p', kI)        = apply ef g' s bs'
    (_  , psu')     = substElim ef (Sol.sEnv s) sEnv g' k su'
    (xts, psu)      = substElim ef (Sol.sEnv s) sEnv g  k su
    su'             = Sol.cuSubst c
    bs              = Sol.cuBinds c
    k               = F.ksuKVar   ksu
...
{- | @substElim@ returns the binders that must be existentially quantified,
 ...
 -}
substElim :: ElabFlags -> F.SymEnv -> F.SEnv F.Sort -> CombinedEnv a -> F.KVar -> F.Subst -> ([(F.Symbol, F.Sort)], F.Pred)
substElim ef syEnv sEnv g _ (F.Su m) = (xts, p)
  where
    p      = F.pAnd [ mkSubst ef sp syEnv x (substSort sEnv frees x t) e t | (x, e, t) <- xets ]
...
-- LH #1091
mkSubst :: ElabFlags -> F.SrcSpan -> F.SymEnv -> F.Symbol -> F.Sort -> F.Expr -> F.Sort -> F.Expr
mkSubst ef sp env x tx ey ty
  | tx == ty    = F.EEq ex ey
  ...
  where
    ...
    ex          = F.expr x
    ex'         = elabToInt ef sp env ex tx
    ey'         = elabToInt ef sp env ey ty

elabToInt :: ElabFlags -> F.SrcSpan -> F.SymEnv -> F.Expr -> F.Sort -> F.Expr
elabToInt ef sp env e s = So.elaborate (So.ElabParam ef (F.atLoc sp "elabToInt") env) (So.toInt env e s)
