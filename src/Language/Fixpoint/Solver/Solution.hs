...
lhsPred
  ...
  -> F.SimpC a
  -> F.Expr
lhsPred cfg scope bindingsInSmt be s c =
    let ap = apply cfg g s bs
     in F.notracepp _msg $ fst ap
  where
    g          = CEnv ci be scope bs (F.srcSpan c) bindingsInSmt
    bs         = F.senv c
    ci         = sid c
...
nonCutsResult :: Config -> S.HashSet F.Symbol -> F.BindEnv ann -> Sol.Sol Sol.QBind -> FixDelayedSolution
nonCutsResult cfg scope be s = M.mapWithKey (\k -> Delayed . mkNonCutsExpr cfg g s k) $ Sol.sHyp s
  where
    g = CEnv Nothing be scope F.emptyIBindEnv F.dummySpan F.emptyIBindEnv
