...
apply g bsInScope s bs
  ...
  | otherwise =
    ...
    let bs' = F.diffIBindEnv bs bsInScope
        xrs = map (lookupBindEnvExt g) (F.elemsIBindEnv bs')
...
applyKVars g bsInScope s ks =
  let (bsCommon0, bs) = commonBindsOfKVars s ks
      bsCommon = F.diffIBindEnv bsCommon0 bsInScope
      bs' = F.diffIBindEnv bs bsInScope
      g' = addCEnv g bsCommon0
      bcs = map (applyKVar g' (F.unionIBindEnv bsInScope bsCommon) s) ks
      (es, is) = unzip bcs
      yts = symSorts g' bsCommon
      (p, _kI) = apply g' mempty s bs'
   in (F.pExist yts (p F.&.& F.pAndNoDedup es), mconcat is)

commonBindsOfKVars :: Sol.Sol Sol.QBind -> [F.KVSub] -> (F.IBindEnv, F.IBindEnv)
commonBindsOfKVars s ks =
  let ibs = [ M.lookupDefault mempty (F.ksuKVar ksu) (Sol.sKVarInvs s) |  ksu <- ks ]
   in (F.unionsIBindEnv ibs, F.unionsIBindEnv $ zipWith F.diffIBindEnv ibs
        [ Misc.safeLookup "sScp" (F.ksuKVar ksu) (Sol.sScp s)
        | ksu <- ks
        ]
      )
...
mkNonCutsExpr ce s k cs =
  let bsCommon = M.lookupDefault mempty k (Sol.sKVarInvs s)
      bs' = F.diffIBindEnv bsCommon (Misc.safeLookup "sScp" k (Sol.sScp s))
      g' = addCEnv ce bs'
      yts = symSorts g' bs'
      bcps = map (bareCubePred g' (F.unionIBindEnv bsCommon (Misc.safeLookup "sScp" k (Sol.sScp s))) s k) cs
      (p, _kI) = apply g' mempty s bs'
...
bareCubePred g bsInScope s k c =
    let (p, _kI) = apply g' bsInScope s bs'
     ...
     in F.pExist yts (F.rapierSubstExpr (F.substSymbolsSet su) su p)
  where
    bs = Sol.cuBinds c
...
hypPred g bsInScope s ksu hyp =
  let k = F.ksuKVar ksu
      bsCommon = M.lookupDefault mempty k (Sol.sKVarInvs s)
