...
applyKVar cfg  g s ksu = case Sol.lookup s (F.ksuKVar ksu) of
  ...
  Right eqs -> let qbp = Sol.qbPreds (F.substFromKSubst $ F.ksuSubst ksu) eqs
                   tySub = kvarTySub s ksu
                   preds = if M.null tySub
                           then fst <$> qbp
                           else V.applyCoSub tySub . fst <$> qbp
...
-- | Compute the type variable substitution for a kvar at a use site
-- by matching the WfC sort against the use-site sort.
kvarTySub :: Sol.Sol Sol.QBind -> F.KVSub -> M.HashMap F.Symbol F.Sort
kvarTySub s ksu = case M.lookup (F.ksuKVar ksu) (Sol.sWfs s) of
  Just (wfSort, tvs) -> F.matchSortsTyVars tvs wfSort (F.ksuSort ksu)
  Nothing -> M.empty
...
cubePred cfg g s ksu c    =
    let (p, kI) = cubePredExc cfg g s c bs'
        ...
        p' = F.rapierSubstExpr (F.substSymbolsSet su) su p
        tySub = kvarTySub s ksu
        p'' = if M.null tySub then p' else V.applyCoSub tySub p'
     in (p'', kI)
  where
    bs' = F.diffIBindEnv bs (Misc.safeLookup "sScp" k (Sol.sScp s))
    bs  = Sol.cuBinds c
