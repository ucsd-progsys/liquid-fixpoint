...
import           Language.Fixpoint.Misc            (safeLookupIntMap, group, errorstar)
...
cutSInfo si kI cKs = si { ws = ws', cm = cm' }
  where
    ...
    cm'   = IntMap.filterWithKey (\i c -> S.member i cs || isConcC c) (cm si)
...
nonCutHyp kI si k = nonCutCube <$> cs
  where
    cs            = getSubC   si <$> M.findWithDefault [] k kI
...
getSubC si i = safeLookupIntMap msg i (cm si)
  where
    msg = "getSubC: " ++ show i
