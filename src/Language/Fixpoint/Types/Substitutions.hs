...
instance (Eq v, Hashable v) => Subable (ExprBV v v) where
  ...
  substf f (PKVar k su tsu)    = PKVar k (mapKVarSubst (substf f) su) tsu
  ...
  subst = go
    where
      ...
      go su e0 = case e0 of
        ...
        PKVar k su' tsu ->
          PKVar k (kSubstFromSubst $ substFromKSubst su' `catSubst` su) tsu
        PAll bs p
          | disjointRange su' bs ->
            PAll bs $ go su' p
          | otherwise ->
...
rapierSubstExpr s su e0 =
  let go = rapierSubstExpr
   in case e0 of
    ...
    PKVar k su' tsu -> PKVar k (catSubstGo su' su) tsu
    PAll bs p ->
