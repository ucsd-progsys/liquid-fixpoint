...
import Language.Fixpoint.Types hiding (GInfo (..), senv, subst)
...
traced str =
  if debugLogs
    then
      let prettified = prettyCallStack (popCallStack callStack)
...
elabExprE msg env e =
  ...
  where
    ...
    d m =
      vcat
        [ "elaborate" <+> text (val msg) <+> "failed on:",
          nest 4 (pprint e),
          "with error",
          nest 4 (text m),
          "in environment",
...
eCstAtom f@(sym, g) (EVar x) t
  | Found s <- g x,
    isUndef s,
    not (isNum sym t) =
      (`ECst` t) <$> elabAs f t (EApp (eVar tyCastName) (eVar x))
...
throwErrorAt ~err' = do
  -- Lazy pattern needed because we use LANGUAGE Strict in this module
...
checkRelTy f _ _ s1@(FObj l) s2@(FObj l')
  | l /= l' =
