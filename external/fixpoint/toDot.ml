...
let t_to_dep t =
  ...
  let src =
	...
	List.flatten |> List.map snd |> List.map Symbol.to_string |> set_of_strings in
  let dst = C.kvars_of_reft rhs |> List.map snd |> List.map Symbol.to_string |> set_of_strings in
    src, tag, dst
