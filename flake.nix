  ...
  outputs = { self, nixpkgs, flake-utils }:
    let
      ...
      mkOutputs = system: {
        ...
        overlays = {
          patchHaskellGit = final: prev: haskellPackagesOverlay final prev (selfH: superH: {
          ...
          addLiquidFixpoint = final: prev: haskellPackagesOverlay final prev (selfH: superH: {
            liquid-fixpoint = prev.haskell.lib.overrideCabal (prev.haskellPackages.callCabal2nix "liquid-fixpoint" self { }) (old: {
              buildTools = [ prev.z3 ];
              doCheck = true;
              doHaddock = true;
